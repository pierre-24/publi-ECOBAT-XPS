...
Another key observation is that slab thickness influences some results more than others. For calculations with \cp{E_F}, \cp{\varepsilon_{Ar,2s}}, and \cp{E_\infty}, the predicted values remain nearly constant as $N$ increases. The SJ\textsuperscript{n} method appears very sensitive to the system size. It should however be noted that with $E_{ref}=\varepsilon_{Ar,2s}$, calculation fails to converge or provide slightly different values, resulting in large standard deviations, notably for CaO with $N=16$ layers. 

Similar trends are observed for O 1s, as shown in Fig.~\ref{fig:slabOH2}, where the simulated XPS spectra for CaO and \ce{CaO.H2O} (both with 6 layers) further emphasize the distinction between bulk and surface contributions. This time, while BE(bulk O 1s in CaO) $>$ BE(surface O 1s in CaO), this is the opposite for \ce{CaO.H2O},  for all protocols.  Furthermore, there is almost no difference in bulk values between the two slab types when using, again, \cp{E_F}, \cp{\varepsilon_{Ar,2s}}, or \cp{E_\infty}, but using $\varepsilon_{Ar,2s}$ leads to unreliable results, as evidenced by the spurious peak at \dbe{} = \SI{0}{\electronvolt} in the O 1s spectra which would indicates a difference between the chemical environment between hydoxyls that is not found using other reference energies.
...
\begin{figure}[!h]
	...
	\caption{Effect of the reference energy on the simulated XPS spectra of CaO (dashed line)  and \ce{CaO.H2O} (plain line), calculated using the different protocols. The horizontal lines indicate the mean values for bulk (``b"), surface (``s"), and surface hydroxides (only for \ce{CaO.H2O}  O 1s) contributions.}
	\label{fig:slabOH2}
\end{figure}
...
\end{inparaenum}
Only the \cp{E_\infty} protocol satisfies these criteria. Consequently, a consistent treatment of both gas-phase systems and slabs using the same method is not feasible.
